...
@section('meta')
    <title>Tourist-attractions.uz - {{ $attraction->{'title_' . $locale} }}</title>
    ...
    <meta property="og:title" content="Tourist-attractions.uz - {{ $attraction->{'title_' . $locale} }}"/>
            ...
            <div class="row">
                <div class="col-xl-8 offset-xl-2 col-lg-10 offset-lg-1 col-md-10 offset-md-1">
                    <div class="breadcrumb-title">
                        <h2>{{ $attraction->{'title_' . $locale} }}</h2>
                        <ul class="breadcrumb-menu">
                            <li><a href="{{ route('home') }}">@lang('main.main') </a>
                                <i class="las la-angle-double-right"></i>
                            </li>
                            <li>@lang('main.attractions')</li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div class="col-xl-8 col-lg-8 col-md-12 col-12">
                    <div class="tour-details-info">
                        <div class="single-img">
                            <img src="{{ asset('storage/' . $attraction->image_2) }}"/>
                        </div>
                        <p>{!! $attraction->{'text_' . $locale} !!}</p>

                        @if($attraction->location)
                            {!!  $attraction->location  !!}
                        @elseif($attraction->lng || $attraction->ltd)
                            <div class="map" id="map"
                                 style="position:relative;overflow:hidden; width: 100%; height: 400px"></div>
                            <script>
                                var placemarks = [
                                    {
                                        latitude: {{$attraction->ltd}},
                                        longitude: {{$attraction->lng}},
                                        {{--hintContent: '{{$attraction->{'address_' . $locale} }}',--}}
                                        {{--balloonContent: '{{$attraction->phone }}',--}}
                                    },
                                ];
